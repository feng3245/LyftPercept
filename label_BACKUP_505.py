...
=======
    binary_car_result = scipy.misc.imresize((im_softmax[0][:, 1] > float(1/3)).reshape(256, 320), (array.shape[0], array.shape[1])).astype('uint8')
    binary_road_result = binary_car_result = scipy.misc.imresize((im_softmax[0][:, 0] > float(1/3)).reshape(256, 320) , (array.shape[0], array.shape[1])).astype('uint8')
>>>>>>> b0c5e6fb033e0e6bd113a8c680157f673bb6ebb3
	#infer model
