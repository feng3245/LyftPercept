...
for f in segfiles:
  ...
  if float(ps)/(im.size[0]*im.size[1])>0.1:
    ...
    newim.save(os.path.dirname(os.path.realpath(__file__))+'/Train/CameraSegAug/Flip'+f)
    imorg = Image.open(orgpath+f).transpose(Image.FLIP_LEFT_RIGHT)
    imorg.save(orgpath+'Flip'+f)
